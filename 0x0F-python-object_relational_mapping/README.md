...
Results are ordered by ascending states.id. 2. Filter states by user input
...
Results are ordered by ascending states.id.
Uses string formatting to construct the SQL query. 3. SQL Injection...
...
Results are sorted by ascending cities.id. 6. First state model
...
Results are sorted by ascending states.id. 8. First state
...
If the states table is empty, prints Nothing. 9. Contains a
...
Results are ordered by ascending states.id. 10. Get a state
...
If no match is found, prints Not found. 11. Add a new state
...
Prints the id of the new State after creation. 12. Update a state
...
Usage: ./12-model_state_update_id_2.py <mysql username> <mysql password> <database name>. 13. Delete states
...
Usage: ./13-model_state_delete_a.py <mysql username> <mysql password> <database name>. 14. Cities in state
...
Results are sorted by ascending cities.id. 15. City relationship
...
Uses the cities relationship for all State objects. 16. List relationship
